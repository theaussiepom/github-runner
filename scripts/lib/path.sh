...
appliance_realpath_norm() {
  # Normalize a path like `realpath -m`.
  # - Does not require the path to exist.
  # - Does not resolve symlinks.
  appliance_realpath_m "$1"
}

appliance_path_is_under() {
  # Returns 0 if candidate is equal to base or strictly under base.
  # Both paths are normalized with appliance_realpath_m.
  local base_raw="$1"
  local candidate_raw="$2"

  local base
  base="$(appliance_realpath_norm "$base_raw")"
  local candidate
  candidate="$(appliance_realpath_norm "$candidate_raw")"
