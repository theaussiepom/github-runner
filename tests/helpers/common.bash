...
setup_test_root() {
	...
	export APPLIANCE_TEST_ORIG_PATH="$PATH"

	# Repo root (for vendored libs, helpers, and stubs). Keep separate from
	# APPLIANCE_ROOT, which is the temp test root for scripts under test.
	if [[ -z "${APPLIANCE_REPO_ROOT:-}" ]]; then
		APPLIANCE_REPO_ROOT="$(cd "$BATS_TEST_DIRNAME/../.." && pwd)"
		export APPLIANCE_REPO_ROOT
	fi

	TEST_ROOT="$(mktemp -d)"
	export TEST_ROOT

	export APPLIANCE_ROOT="$TEST_ROOT"
	export APPLIANCE_CALLS_FILE="$TEST_ROOT/calls.log"
...
make_isolated_path_with_stubs() {
	...
	local stub
	for stub in "$@"; do
		cp "$APPLIANCE_REPO_ROOT/tests/stubs/$stub" "$bin_dir/$stub"
...
teardown_test_root() {
	...
	if [[ -n "${APPLIANCE_TEST_ORIG_PATH:-}" ]]; then
		export PATH="$APPLIANCE_TEST_ORIG_PATH"
	fi
